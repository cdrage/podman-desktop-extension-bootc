...
const bootcImageBuilderName = 'quay.io/centos-bootc/bootc-image-builder:latest';
    ...
    async progress => {
      ...
      try {
        ...
        const buildImageContainer = createBuilderImageOptions(
          image.name + bootcImageBuilderContainerName,
          image.name,
          selectedType,
          selectedFolder,
          imagePath,
        ...
        await containerUtils.removeContainerIfExists(buildImageContainer.name);

        // Step 3. Create and start the container for the actual build
        progress.report({ increment: 6 });
        const containerId = await containerUtils.createAndStartContainer(buildImageContainer);
        ...
        await containerUtils.waitForContainerToExit(buildImageContainer.name);
      ...
      // Only if success = true and type = ami or raw
